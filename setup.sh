...
bash -c 'yay -S --mflags --skipinteg --answerclean All --answerdiff All --answeredit All --answerupgrade Repo ani-cli chocolate-doom tlpui-git spotify picom-jonaburg-git github-desktop-bin mirage libinput-gestures sublime-text-4 bashmount spotify-tui trackma'

# Move configs and files to directories
bash -c 'mkdir ~/.xmonad && cp xmonad.hs ~/.xmonad/'
bash -c 'mkdir ~/.xmobarrc && cp xmobar.hs ~/.xmobarrc/xmobar.hs'
